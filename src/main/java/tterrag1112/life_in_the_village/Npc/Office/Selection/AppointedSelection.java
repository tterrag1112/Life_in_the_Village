...
public final class AppointedSelection implements OfficeSelectionEngine {
    ...
    public Optional<OfficeHolding> select(OfficeSelectionContext ctx) {
        ...
        // Phase 6.3.3.b.4 — BUSINESS_OWNER is no longer an office holding;
        // the appointer for business-tier offices is the Business owner
        // resolved directly from BusinessSavedData.
        UUID appointerNpcId;
        if (OfficeRegistry.BUSINESS_OWNER.equals(appointerId)) {
            tterrag1112.life_in_the_village.Guilds.Companies.Business biz =
                    tterrag1112.life_in_the_village.Guilds.Companies.BusinessSavedData
                            .get(ctx.level())
                            .getById(ctx.orgId())
                            .orElse(null);
            if (biz == null || !biz.isNpcOwned()) return Optional.empty();
            appointerNpcId = biz.getOwnerId();
        } else {
            ...
            appointerNpcId = appointerHolding.holderNpcId().orElse(null);
        }
