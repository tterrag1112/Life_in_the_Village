...
| ID | Task | Subsystem | Status | Notes |
|---|---|---|---|---|
| P1-01 | (legacy id — superseded by polygon plaza rollout below) | 04-town-square-rework | Superseded | Old `TownSquareComposer` shipped here; prompt 15 audit found it stamps a paved square on top of unaware roads. Re-scoped into P1-01a / P1-02 / P1-03 (prompt 17) / P1-04 (prompt 18). Composer code stays in place via prompt 16 with palette switch; full removal in prompt 17. |
| P1-02 | (legacy id — see P1-02b below) | 04-town-square-rework | Superseded | Sub-slot emission landed correctly in prompt 14 and is kept by the redesign. The mechanism (`emitPlazaSubSlots` on the slot emitter, `PARK_FEATURE + PLAZA_*` sub-tags) carries forward; prompt 17 will replace its radius-based footprint with polygon-aware sampling. |
| P1-03 | (legacy id — superseded) | 04-town-square-rework | Superseded | `TownSquareKit` + `TownSquareKitRegistry` shipped here; the polygon model deletes the kit-registry layer and uses direct `DecorationProfile` registrations. Removal lands in prompt 17. |
| P1-04 | (legacy id — see P1-04b below) | 04-town-square-rework | Superseded | `GatheringPoint` registration landed here and is kept by the redesign — prompt 17 reroutes producer to polygon edges, but the consumer-facing API (Village.getGatheringPoints) is stable. |
| P1-05 | (legacy id — superseded) | 04-town-square-rework | Superseded | Tier→plazaRadius migration in `LayoutPrimitive.TownSquare` shipped here. The layout's `townSquareRadius` field is retained but becomes incidental once polygon plazas land in prompt 17 (the polygon, not the radius, drives downstream geometry). |
| P1-01a | Plaza paving palette switch | 04-town-square-rework | Implemented (prompt 16) | `TownSquareComposer.pavePlaza` now resolves `PathMaterial.forBiomeAndTier(style, village.getPathTier())` (the same source the road realiser uses) and samples interior blocks via `material.sampleCore(random)` and the outer ring via `material.sampleEdge(random)`. The legacy `style.stone`/`stoneSlab`/`pathState` triplet is gone; the inner slab ring is dropped because it doesn't exist anywhere else in the road network and re-introduced the "stamped square" cue. Plaza paving is now visually continuous with surrounding roads even though the plaza is still architecturally an axis-aligned square. |
| P1-01b | PlazaRegion data model + Polygon utility | 04-town-square-rework | Implemented (prompt 16) | New package `Village/Decoration/Plaza/` with `PlazaRegion` (record + codec, UUID-keyed, polygon footprint, centroid, floorY, connectedRoadIds), `PlazaPurpose` enum (CIVIC/MARKET/RELIGIOUS_COURTYARD), `PlazaShape` enum (CIRCLE/SQUARE/LINEAR/IRREGULAR), `VillageCenterMarker` (HAMLET-tier alternative). New `Utilities/Geometry/Polygon.java` (~280 LOC) with `contains` (ray casting), `distanceToEdge` (point-segment with clamp), `area` (shoelace), `boundingBox`, `simplify` (Douglas-Peucker on closed polygon split at extreme-distance vertices), `centroid` (signed-area weighted). Default DP tolerance discussion: 1.5 blocks recommended for IRREGULAR plazas in doc 04 — the helper takes tolerance per-call rather than baking it in. |
| P1-01c | PlanContext + Village codec extension | 04-town-square-rework | Implemented (prompt 16) | `PlanContext` gains `plazaRegions` list + `villageCenter` field with accessors `addPlazaRegion`, `getPlazaRegions`, `getPlazaRegionContaining(pos)`, `getPlazaRegionNear(pos, distance)`, `setVillageCenter`, `getVillageCenter`. No producers in prompt 16. Persistence on `Village` via a new `VillagePlazaMeta` sub-record (16th group field on `Village.CODEC`); kept separate from `VillageLayoutMeta` because the latter is at 14 of 16 fields and would hit the codec ceiling. Both fields use `optionalFieldOf` defaults; pre-prompt-16 saves load with empty plaza state. |
| P1-01d | `PLAZA_ADJACENT` slot tag | 04-town-square-rework | Implemented (prompt 16) | Appended to `SlotTag` enum (plain Java enum, no codec ceremony). No emitter today; prompt 17's polygon generator emits along polygon edges, prompt 18's civic placement matcher reads the tag. |
| P1-01e | DUAL_PLAZA workaround flag | 04-town-square-rework | Implemented (prompt 16) | `DualPlazaRecipe.compose` gains a `TODO(prompt 17)` comment pointing at the polygon-based fix. The brittle "second TownSquare overwrites civicRingRadius, recipe saves sq1Ring locally" workaround stays in place until prompt 17 lands per-plaza geometry. |
| P1-02b | Polygon plaza generation + paver | 04-town-square-rework | Implemented (prompt 17) | `PlazaGenerator` (4 shapes: CIRCLE 16-gon, SQUARE 70/30 cardinal/diagonal, LINEAR rectangle on cardinal axis, IRREGULAR perturbed circle + Douglas-Peucker simplify); `PlazaPaver` runs in `VillageDecorator` after road network using same `PathMaterial.sampleCore` + `sampleEdge` palette. `RecipeHelpers.installPlaza` one-line call wired into all 17 recipes (CIRCLE: RADIAL/PLAZA; SQUARE: CROSSROADS/ENCLAVE/HILLTOP/OUTPOST; LINEAR: LINEAR/CHAIN/RIVERINE/DOCKSIDE/ROADSIDE/TERRACED; IRREGULAR: CLUSTERED/SPRAWL/GROVE/DUMBELL; DUAL_PLAZA: CIRCLE primary + SQUARE secondary). HAMLET tier registers `VillageCenterMarker` instead of polygon. Terrain accommodation receeds vertices from water + steep slope (single per-vertex pass); building-recession deferred to prompt 18 because committed buildings don't exist at compose time. `connectedRoadIds` left empty until road UUIDs exist post-realisation. PLAZA_ADJACENT slot emission landed (no consumer until prompt 18). Plaza polygon + center marker plumbed PlanContext → VillageLayout → Village.applyLayout → persisted Village. `DecorationSlotEmitter.emitPlazaSubSlots` is polygon-aware: derives effective centre + radius from polygon centroid + sqrt(area/π) when registered, falls back to legacy `townSquareRadius` otherwise. `TownSquareComposer` retained (its stamped-square paving overlaps the polygon at centre with the same palette — no visible duplication). |
| P1-04b | Civic placement migration + composer cleanup | 04-town-square-rework | Implemented (prompt 18) | Plaza-aware building rotation: `PlanContext.tryCommitBuilding` consults `getPlazaRegionNear(target, 8)` before the road-facing fallback. Civic slot emission moved into `PlazaGenerator.emitPlazaCivicSlots` (PRIME_CIVIC for first slot, SECONDARY_CIVIC for rest, all also tagged PLAZA_ADJACENT + ROAD_ADJACENT, outset 8 matching legacy `civicInnerEdge + maxCivicFrontFace/2` standoff). `LayoutPrimitive.TownSquare` deleted entirely (removed from sealed-interface permits; body deleted ~140 LOC). 12 recipe call sites cleared. `TownSquareComposer` + `TownSquareKit` + `TownSquareKitRegistry` + `DefaultTownSquareKits` deleted; `VillageDecorator` no longer calls composer; `Life_in_the_village.commonSetup` no longer registers default kits. Gathering point registration migrated from composer to `PlazaGenerator.registerGatheringPoints`. Sub-slot position helpers inlined into `DecorationSlotEmitter` as `PlazaSubSlotPositions` nested class. DUAL_PLAZA `sq1Ring` workaround deleted. Ring road (`RoadPrimitive.Ring`) deleted — purely decorative per prompt-18 audit, no NPC / trade / weathering dependency. `installPlaza` is the unified entry point: sets every layout state field recipes / emitter consume (townSquarePos / townSquareRadius / civicRingRadius / forced DECORATION reserve / civic slots / gathering points). HAMLET tier registers VillageCenterMarker + sets fields with radius-3 defaults. No civic profile changes — they already require PRIME_CIVIC / SECONDARY_CIVIC, which polygon-edge slots now provide. |
