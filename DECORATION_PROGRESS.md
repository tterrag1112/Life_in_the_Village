...
| ID | Task | Subsystem | Status | Notes |
|---|---|---|---|---|
| P1-01 | `TownSquareComposer` replaces TownSquarePlacer | 04-town-square-rework | Implemented | `TownSquareComposer.compose(level, center, style, tier, village, data)` is a drop-in for the legacy `TownSquarePlacer.place(...)` (same signature, same return). Tier-scaled paving via `VillageBiomeStyle` (programmatic, not NBT — see doc 04 revision notes). Registers a `BuildingType.TOWN_SQUARE` synthetic building like the legacy code. `TownSquarePlacer` reduced to event-decoration helpers (`decorateForEvent` for VILLAGE_FAIR / MARKET_DAY / etc.); its `place()` method removed and four call sites migrated. |
| P1-02 | Sub-slot emission (fountain, monument, benches, etc.) | 04-town-square-rework | Implemented | `DecorationSlotEmitter.emitPlazaSubSlots` adds the 7th sub-algorithm to the uniform emit pass, sized off `village.getTownSquareRadius()`. Eight sub-roles: PLAZA_FOUNTAIN, PLAZA_MONUMENT, PLAZA_GAZEBO, PLAZA_BENCH (×up-to-4), PLAZA_NOTICE_BOARD, PLAZA_LAMP (×4), PLAZA_FLOWERBED, PLAZA_VENDOR_ZONE. Doc 04 originally proposed a `PlazaSubSlot` field on `DecorationSlot`; we instead extended `DecorationTag` with `PLAZA_*` values so the existing tag-filter machinery handles sub-role matching with no new field — documented in revision notes. Sub-slot positions delegate to public `TownSquareComposer` position helpers so emitter and gathering-point registration agree. |
| P1-03 | Per-tier square kit selection | 04-town-square-rework | Implemented | `TownSquareKit` record + `TownSquareKitRegistry` indexed by `(culture, tier)` with the doc 04 fallback chain (exact → default-culture-same-tier → same-culture-larger-tier → default-larger-tier). `DefaultTownSquareKits.registerAll()` is called from `commonSetup` and registers four default kits (HAMLET / VILLAGE / TOWN / CITY) plus the corresponding `DecorationProfile`s. Profiles dedup by `pieceId` so pieces shared across tiers (benches, gazebo) don't fire duplicate-registration warnings. Doc 04's `pavingNbtOrNull` field omitted — paving is biome-and-culture-styled programmatic placement, not an NBT (revision note). The actual NBT pieces are user-authored; the registry points at the canonical paths and the matcher logs missing-NBT warnings for any not yet authored. |
| P1-04 | Gathering-point registration for NPC hobbies | 04-town-square-rework | Implemented | `GatheringPoint` record + `GatheringPointKind` enum (BENCH/FOUNTAIN/GAZEBO/NOTICE_BOARD/VENDOR_ZONE) live in the TownSquare package. Persisted on `Village` via the existing `VillageLayoutMeta` sub-record (added two fields: `townSquareRadius` and `gatheringPoints`, both `optionalFieldOf` so pre-Phase-1 saves load with empty defaults). Composer registers one point per placed sub-feature with doc 04 capacities (BENCH 1, FOUNTAIN 3, GAZEBO 5, NOTICE_BOARD 1, VENDOR_ZONE 4). Gathering-point UUIDs are deterministic from `(villageId, plaza pos, tag)` via `nameUUIDFromBytes` so re-emission yields stable ids. NPC consumption deferred to NPC Phase 2 hobby system. |
| P1-05 | Existing plaza geometry migration | 04-town-square-rework | Implemented | `LayoutPrimitive.TownSquare` now resolves plaza half-extent via `TownSquareTier.plazaRadiusFor(pctx.sizeTier())` instead of `TownSquarePlacer.RADIUS + 2`. Tier mapping: HAMLET=3 (matches the legacy value byte-for-byte, so existing seeds reproduce exactly), VILLAGE=5, TOWN=8, CITY=12. Two reads in LayoutPrimitive replaced (`place()` and `emitSlots()`); the fallback in `VillagePlanner` (when a recipe never set the square pos) uses `TownSquareTier.RADIUS_HAMLET` explicitly. `VillageDecorator` reads `village.getTownSquareRadius()` for footprint occupy instead of the literal `TownSquarePlacer.RADIUS`. Downstream consumers of `ringRoadRadius`, `civicInnerEdge`, `placementRing` are unaffected — they derive from `plazaRadius` via existing formulae. |
