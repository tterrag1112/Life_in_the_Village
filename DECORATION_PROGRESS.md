...
| P0a-08 | `Building` record fields: variantId, primaryColor, accentColor, roofColor | 15-building-variants-and-colors | Implemented | All four added; codec uses `optionalFieldOf` for forward-compat. `variantId` defaults to type-default in both constructors. Save-data migration (P0a-20) is still pending. |
