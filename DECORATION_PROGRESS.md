...
| P1-02b | Polygon plaza generation | 04-town-square-rework | Deferred (prompt 17) | Polygon generator produces `PlazaRegion`s per layout shape; `PlazaPaver` runs alongside `VillageRoadNetwork` using the same `PathMaterial`; `TownSquareComposer` retired; sub-slot emission switches to polygon-aware sampling. |
